...
function positionTutorialCard(target, placement) {
  ...
  if (placement === 'top') {
    card.style.top = `${Math.max(24, rect.top - 220)}px`;
    card.style.bottom = 'auto';
    return;
  }

  if (placement === 'left') {
    card.style.top = `${Math.min(window.innerHeight - 280, Math.max(24, rect.top))}px`;
    card.style.bottom = 'auto';
    card.style.right = `${Math.max(24, window.innerWidth - rect.left + 18)}px`;
    return;
  }

  card.style.top = `${Math.min(window.innerHeight - 280, rect.bottom + 18)}px`;
  card.style.bottom = 'auto';
}
