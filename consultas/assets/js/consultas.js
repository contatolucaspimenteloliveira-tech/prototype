...
const defaultConfig = {
  ...
  hero_subtitle: 'Tudo em um só lugar. Rápido, simples e gratuito.'
};
...
async function consultarCNPJ() {
  ...
  try {
    ...
    document.getElementById('cnpj-data').innerHTML = `
      <div class="space-y-3">
        <div><span class="text-xs text-slate-500 uppercase">Razão Social</span><p class="text-slate-200 font-medium text-lg">${data.name || 'N/A'}</p></div>
        <div class="grid grid-cols-2 gap-4">
          <div><span class="text-xs text-slate-500 uppercase">CNPJ</span><p class="text-slate-200 font-medium">${cnpj}</p></div>
          <div><span class="text-xs text-slate-500 uppercase">Situação</span><p class="text-slate-200 font-medium">${data.status || 'N/A'}</p></div>
        </div>
        <div><span class="text-xs text-slate-500 uppercase">Endereço</span><p class="text-slate-200 font-medium">${data.street || 'N/A'}, ${data.number || 'N/A'} - ${data.city || 'N/A'}</p></div>
      </div>
    `;
  } catch {
...
async function consultarPlaca() {
  ...
  document.getElementById('placa-data').innerHTML = `
    <div class="text-center p-4 bg-yellow-500/20 rounded-lg border border-yellow-500/30">
      ...
      <p class="text-slate-400 text-sm mt-2">Em breve será possível consultar informações de placas</p>
    </div>
  `;
}
